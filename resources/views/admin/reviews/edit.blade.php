...
@section('header_title', 'Edit Product Review')

@section('content')
<div class="max-w-xl mx-auto bg-white rounded-2xl border border-slate-100 p-6 sm:p-8 shadow-xs">
    
    <div class="mb-6 flex items-center justify-between">
        <h3 class="font-serif font-bold text-slate-800 text-lg">Modify Product Review</h3>
    ...
    <form method="POST" action="{{ route('admin.reviews.update', $review->id) }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="space-y-1.5">
                ...
                <input type="email" name="reviewer_email" value="{{ old('reviewer_email', $review->reviewer_email) }}"
                       class="w-full border border-slate-200 focus:ring-1 focus:ring-[#C49A6C] focus:border-[#C49A6C] rounded-xl text-sm px-4 py-2.5">
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div class="space-y-1.5">
                ...
                <select name="rating" required class="w-full border border-slate-200 focus:ring-1 focus:ring-[#C49A6C] focus:border-[#C49A6C] rounded-xl text-sm px-4 py-2.5 bg-white">
                    <option value="5" @selected(old('rating', $review->rating) == 5)>5 Stars</option>
                    <option value="4" @selected(old('rating', $review->rating) == 4)>4 Stars</option>
                    <option value="3" @selected(old('rating', $review->rating) == 3)>3 Stars</option>
                    <option value="2" @selected(old('rating', $review->rating) == 2)>2 Stars</option>
                    <option value="1" @selected(old('rating', $review->rating) == 1)>1 Star</option>
                </select>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <label class="flex items-center space-x-3 cursor-pointer border border-slate-200 rounded-xl px-4 py-2.5 bg-white h-[45px]">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', $review->is_active))
        ...
        <div class="space-y-1.5">
            ...
            <textarea name="review" rows="4" required
                      class="w-full border border-slate-200 focus:ring-1 focus:ring-[#C49A6C] focus:border-[#C49A6C] rounded-xl text-sm px-4 py-2.5">{{ old('review', $review->review) }}</textarea>
        </div>
